...
const deleteAvatarFlag = ref(false); // Variable para borrar foto
...
const saveProfile = async () => {
    try {
        ...
        // Si borramos la foto
        else if (deleteAvatarFlag.value) {
        ...
        // Mantener datos calculados localmente
        ...
        <div v-else-if="user" class="profile-card">
            ...
            <div class="profile-header">
                ...
                <div class="avatar-container" :class="{ 'editable': isEditing }"
                    ...
                    <img v-else-if="user.avatar_url && !deleteAvatarFlag"
                ...
                <button v-if="isEditing && (user.avatar_url || imagePreview) && !deleteAvatarFlag" @click="removeAvatar"
                ...
                <div v-if="!isEditing" class="header-info">
                    ...
                    <div class="role-badge-container">
                        <span v-if="['seller', 'vendedor'].includes(user.role)" class="badge seller">Vendedor
                            Verificado</span>
            ...
            <div class="profile-body">
                ...
                <form v-if="isEditing" @submit.prevent="saveProfile" class="edit-form">
                    <div class="form-group"><label>Nombre</label><input v-model="form.name" type="text" required
                            class="input-field" /></div>
                    <div class="form-group"><label>Apellidos</label><input v-model="form.surname" type="text" required
                            class="input-field" /></div>
                    <div class="form-group"><label>Email</label><input v-model="form.email" type="email" required
                            class="input-field" /></div>
                ...
                <div v-else class="info-view">
                    ...
                    <div class="info-row"><label>Email</label>
                        <p>{{ user.email }}</p>
                    </div>
                    ...
                    <div class="info-row"><label>Miembro desde</label>
                    ...
                    <div class="main-actions">
                        ...
                        <template v-if="['seller', 'vendedor'].includes(user.role)">
                            ...
                            <button @click="goToPickupPoints" class="btn btn-pickup"> Gestionar Puntos de
                                Recogida</button>
                        </template>

                        <button v-if="!['seller', 'vendedor'].includes(user.role)" @click="showSellerModal = true"
                            class="btn btn-become-seller"> ¡Quiero Vender!</button>
        ...
        <div v-if="showSellerModal" class="modal-overlay">
            <div class="modal-content">
                ...
                <form @submit.prevent="becomeSeller" class="modal-body">
                    <div class="form-group"><label>Nombre</label><input v-model="sellerForm.store_name" type="text"
                            required class="input-field"></div>
                    <div class="form-group"><label>NIF</label><input v-model="sellerForm.nif" type="text" required
                            class="input-field"></div>
                    <div class="form-group"><label>Descripción</label><textarea v-model="sellerForm.description"
                            rows="3" class="input-field"></textarea></div>
                    <div class="modal-actions"><button type="button" @click="showSellerModal = false"
        ...
        <div v-if="showReviewsModal" class="modal-overlay" @click.self="showReviewsModal = false">
            <div class="modal-content reviews-modal-content">
                ...
                <div class="modal-actions" style="justify-content: center; margin-bottom: 25px;">
                    <button @click="showReviewsModal = false" class="btn btn-secondary"
                        style="width: auto;">Cerrar</button>
                </div>
            </div>
        </div>
...
/* Nuevo estilo para el botón de texto */
.btn-remove-text {
    display: block;           /* Para que ocupe su propia línea */
    margin: -10px auto 15px;  /* Centrado y ajustando márgenes */
